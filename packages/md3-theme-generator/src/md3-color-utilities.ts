...
export async function sourceColorFromImagePath(path: string) {
  ...
  const imageBytes = await new Promise<Uint8ClampedArray>((resolve, reject) => {
    ...
    const callback = () => {
      ...
      resolve(
        context.getImageData(Number(sx), Number(sy), Number(sw), Number(sh))
          .data
      );
    };
    if (image.complete) {
  ...
  for (let i = 0; i < imageBytes.length; i += 4) {
    ...
    const argb = argbFromRgb(Number(r), Number(g), Number(b));
    pixels.push(argb);
  }
...
export async function themeFromImagePath(
  ...
  return themeFromSourceColor(source as number, customColors);
}
